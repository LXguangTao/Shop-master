...
using RestSharp;//Nuget使用104.5.0版本
...
namespace Utilities
{
    ...
    {
        //浏览器列表
        private static readonly string[] Browserlist = { "ie", "chrome", "mozilla", "netscape", "firefox", "opera", "konqueror" };
        //搜索引擎列表
        private static readonly string[] Searchenginelist = { "baidu", "google", "360", "sogou", "bing", "msn", "sohu", "soso", "sina", "163", "yahoo", "jikeu" };
        //meta正则表达式
        ...
        public static string GetIP()
        {
            ...
            if (string.IsNullOrEmpty(ip) || ! StringHelper.RegexIP.IsMatch(ip))
                ip = "127.0.0.1";
            return ip;
        }
        ...
        /// <returns></returns>
        public static string GetRequestData(SortedDictionary<string, string> postData, string url)
        {
            return GetRequestData(url, Method.POST, postData);
        }
        ...
        /// <param name="postData">发送数据</param>
        /// <returns></returns>
        public static string GetRequestData(string url, Method method, SortedDictionary<string, string> postData)
        {
        ...
        /// <returns></returns>
        public static string GetRequestData(string url, Method method, SortedDictionary<string, string> postData, Encoding encoding)
        {
        ...
        /// <returns></returns>
        public static string GetRequestData(string url, Method method, SortedDictionary<string, string> postData, Encoding encoding, int timeout)
        {
            ...
            var request = new RestRequest(method) { Timeout = timeout };
            ...
            if (postData != null && postData.Any())
            {
                ...
                {
                    request.AddParameter(item, postData[item], ParameterType.GetOrPost);
                }
            }
            //执行
